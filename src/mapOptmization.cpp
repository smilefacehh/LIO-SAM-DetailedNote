...
--------------------------------------------------
功能简介:
    1、scan-to-map匹配：提取当前激光帧特征点（角点、平面点），局部map特征点，执行scan-to-map迭代优化，更新当前帧位姿
    2、关键帧因子图优化：关键帧则构成因子图，加入激光里程计因子、GPS因子、闭环因子，执行因子图优化，更新所有关键帧位姿
    3、闭环检测：在历史关键帧中找距离相近，时间相隔较远的帧设为匹配帧，匹配帧周围提取局部map，同样执行scan-to-map匹配，得到位姿变换，构建闭环因子数据，加入因子图优化
